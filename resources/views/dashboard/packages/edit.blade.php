    ...
    <form action="{{ route('dashboard.packages.update', $package->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $package->name) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            
            <div class="flex items-center pt-6">
                <input type="checkbox" name="is_popular" id="is_popular" value="1" {{ old('is_popular', $package->is_popular) ? 'checked' : '' }} class="h-5 w-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" name="price" value="{{ old('price', $package->price) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Satuan/Periode</label>
                <input type="text" name="period" value="{{ old('period', $package->period) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Keterangan Cicilan</label>
                <input type="text" name="payment_terms" value="{{ old('payment_terms', $package->payment_terms) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Teks Tombol</label>
                <input type="text" name="button_text" value="{{ old('button_text', $package->button_text) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Link WhatsApp Tombol</label>
                <input type="text" name="button_link" value="{{ old('button_link', $package->button_link) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
        </div>
            ...
            <div id="features-container" class="space-y-3">
                ...
                @if(is_array($package->features) && count($package->features) > 0)
                    ...
                    <div class="feature-row flex items-center gap-3">
                        <input type="text" name="features[{{ $featureIndex }}][text]" value="{{ $feature['text'] }}" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        <select name="features[{{ $featureIndex }}][is_active]" class="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <option value="1" {{ isset($feature['is_active']) && $feature['is_active'] ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ isset($feature['is_active']) && !$feature['is_active'] ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                ...
                @else
                    <div class="feature-row flex items-center gap-3">
                        <input type="text" name="features[0][text]" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
        ...
        <div class="flex justify-end gap-3 border-t pt-6">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium">Simpan Perubahan</button>
        </div>
    </form>
</div>
